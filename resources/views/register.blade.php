...
@section("main")
    <div id="l_r_form">
        ...
        <form action="{{url("register/confirm")}}" method="post">
            @csrf
            @if(!empty($errors->first("user_name"))) <p class="form_error_message">{{$errors->first("user_name")}}</p> @endif
            <label>
                <input class="l_r_form @if(!empty($errors->first("user_name"))) has-error @endif" type="text" name="user_name" placeholder="名前" value="{{old("user_name",@$user_name ?: "")}}"
                       required>
            ...
            <label>
                <input class="l_r_form @if(!empty($errors->first("email"))) has-error @endif" type="text" name="email" placeholder="メールアドレス" value="{{old("email",@$email ?: "")}}" required>
